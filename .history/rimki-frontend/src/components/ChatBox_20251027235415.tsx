    ...
    >
      <div className="chatbot-body">
        {messages.map((msg, index) => (
          <Space
            key={index}
            className={`chat-message ${msg.sender === "User" ? "user" : "rimki"}`}
            align="start"
          >
            {msg.sender === "RIMKI" && (
        ...
        <Space.Compact className="chat-input" block>
          <TextArea
            placeholder={isAuthenticated ? "Ask anything" : "Please login to send messages"}
        ...
        {!isAuthenticated && (
          <div style={{ marginTop: "20px" }}>
            <LoginPrompt
              ...
              message="Please log in to chat with RIMKI Assistant and get help with your security policies."
            />
          </div>
        )}
      </div>
